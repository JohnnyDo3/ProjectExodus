...
export function MarkdownContent({ content }: MarkdownContentProps) {
  ...
  const convertMarkdownToHTML = (markdown: string): string => {
    ...
    // Lists - Ordered
    html = html.replace(/^\d+\.\s+(.+)$/gm, '<li class="ml-6 mb-2 font-medium" style="color: #333">$1</li>')
    ...
    html = html.split('\n\n').map(para => {
      ...
      if (para.trim() === '') return ''
      return `<p class="text-lg font-medium leading-relaxed mb-6" style="color: #333">${para}</p>`
    }).join('\n')
