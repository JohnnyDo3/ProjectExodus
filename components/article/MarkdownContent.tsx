...
export function MarkdownContent({ content }: MarkdownContentProps) {
  ...
  const convertMarkdownToHTML = (markdown: string): string => {
    ...
    // Lists - Ordered
    html = html.replace(/^\d+\.\s+(.+)$/gm, '<li class="ml-6 mb-2 font-semibold" style="color: #111">$1</li>')
    html = html.replace(/(<li.*<\/li>\n?)+/g, '<ol class="list-decimal list-outside space-y-2 mb-6">$&</ol>')
    ...
    html = html.split('\n\n').map(para => {
      ...
      if (para.trim() === '') return ''
      return `<p class="text-lg font-semibold leading-relaxed mb-6" style="color: #111">${para}</p>`
    }).join('\n')
