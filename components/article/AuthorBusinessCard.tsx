...
interface AuthorBusinessCardProps {
  author: {
    id: string
    name: string | null
    email?: string
...
export function AuthorBusinessCard({ author }: AuthorBusinessCardProps) {
  ...
  const archetypeKey = (author.guardianArchetype || 'michael') as ArchetypeType
  const archetype = GUARDIAN_ARCHETYPES[archetypeKey] || GUARDIAN_ARCHETYPES.michael
  const ArchetypeIcon = archetype.icon

  // Calculate STOCK score
  const stockScore = author._count
    ? (author._count.createdProjects * 10) +
      (author._count.articles * 5) +
      (author._count.followers * 1) +
      (author._count.projectMemberships * 2)
    : 0
  ...
  const scrollToBusinessCard = () => {
    if (businessCardRef.current && scrollContainerRef.current) {
      ...
      const scrollOffset = container.scrollTop + (cardTop - containerTop) - 10
  ...
  const handleMessage = () => {
    ...
    router.push(`/messages?user=${author.id}`)
  }
          ...
          <div ref={profileSectionRef} className="p-6">
            ...
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {author.headline && (
                  <p className="text-sm font-semibold text-theme-primary line-clamp-1">
                    {author.headline}
                  </p>
                )}
                {author.location && (
            ...
            {author._count && (
              <div className="grid grid-cols-2 gap-4 mb-4 p-4 bg-[var(--muted)] rounded-lg">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs font-bold text-theme-muted uppercase">Articles</p>
                </div>
              </div>
            )}
            ...
            <div className={`p-3 bg-gradient-to-r ${archetype.gradient} relative overflow-hidden`}>
              ...
              <div className="relative z-10 flex items-center gap-3">
                ...
                <div className="relative w-10 h-10 rounded-full border-2 border-white/50 overflow-hidden bg-white/20 flex items-center justify-center">
                  {author.image ? (
                    <Image src={author.image} alt={author.name || 'Author'} fill unoptimized sizes="100%" className="object-cover" />
                  ) : (
                ...
                <div className="flex-1 min-w-0 text-white">
                  ...
                  <p className="text-[9px] font-bold opacity-80 truncate">
                    {author.headline || archetype.title}
                  </p>
                </div>
