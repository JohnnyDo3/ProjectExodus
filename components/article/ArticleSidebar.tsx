...
export function ArticleSidebar({
  ...
  return (
    <aside className={`space-y-6 ${isDiscussionExpanded ? 'fixed inset-0 z-50 bg-[var(--background)] p-6 overflow-y-auto' : 'md:col-span-1'}`}>
      {/* Expanded header when discussion is full screen */}
      {isDiscussionExpanded && (
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-2xl font-black text-[var(--foreground)]">Article Discussion</h2>
          <button
      ...
      {/* References Widget */}
      {!isDiscussionExpanded && references.length > 0 && (
      ...
      {/* Discussion Widget */}
      <Card className={`border-2 border-theme-primary ${isDiscussionExpanded ? 'h-full flex flex-col' : ''}`}>
        <CardHeader className="pb-2">
        ...
        <CardContent className={`space-y-4 ${isDiscussionExpanded ? 'flex-1 overflow-y-auto' : ''}`}>
          {/* Comment Input */}
          {session?.user ? (
          ...
          {/* Comments List */}
          <div className={`space-y-4 ${isDiscussionExpanded ? '' : 'max-h-64 overflow-y-auto'}`}>
      ...
      {/* Author Card - Hidden when expanded */}
      {!isDiscussionExpanded && author && (
        <Card>
          <CardHeader className="pb-2">
      ...
      {/* Share Card - Hidden when expanded */}
      {!isDiscussionExpanded && (
        <Card>
          <CardHeader className="pb-2">
